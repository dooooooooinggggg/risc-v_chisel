...
// Instruction Decode (ID) Stage
    ...
    val rs1_data = Mux((rs1_addr =/= 0.U(WORD_LEN.U)), regfile(rs1_addr),
    0.U(WORD_LEN.W))
    val rs2_data = Mux((rs2_addr =/= 0.U(WORD_LEN.U)), regfile(rs2_addr),
    0.U(WORD_LEN.W))
    ...
    val exe_fun :: op1_sel :: op2_sel :: mem_wen :: rf_wen :: wb_sel :: csr_cmd ::
    Nil = csignals
    ...
    printf(p"io.pc : 0x${Hexadecimal(pc_reg)}・n")
    printf(p"inst : 0x${Hexadecimal(inst)}・n")
    printf(p"gp : ${regfile(3)}・n")
    printf(p"rs1_addr : $rs1_addr・n")
    printf(p"rs2_addr : $rs2_addr・n")
    printf(p"wb_addr : $wb_addr・n")
    printf(p"rs1_data : 0x${Hexadecimal(rs1_data)}・n")
    printf(p"rs2_data : 0x${Hexadecimal(rs2_data)}・n")
    printf(p"wb_data : 0x${Hexadecimal(wb_data)}・n")
    printf(p"dmem.addr : ${io.dmem.addr}・n")
    printf(p"dmem.rdata : ${io.dmem.rdata}・n")
    printf("---------・n")
}